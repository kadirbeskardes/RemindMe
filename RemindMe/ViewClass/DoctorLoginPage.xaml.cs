    ...
    public partial class DoctorLoginPage : ContentPage
    {
        ...
        private async void Button_Clicked_1(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var islog = (await client.isLogDoctor(idEntry.Text, getHash(passwordEntry.Text)));
                    if (islog)
                    {
                        App.Current.MainPage = new DoctorMainPage(idEntry.Text);
                    }
                    else
                    {
        ...
        {
            App.Current.MainPage = new NavigationPage(new DoctorCreate());
        }
    }
}